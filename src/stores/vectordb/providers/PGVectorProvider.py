...
class PGVectorProvider(VectorDBInterface):
    def __init__(self, db_client, default_vector_size: int = 786,
        ...
        # Add postfix to the collection name so can index collection name easier:
    ...
    async def is_collection_existed(self, collection_name: str) -> bool:
        ...
        async with self.db_client() as session:
            async with session.begin():
                list_stmt = sql_text(f'SELECT * FROM pg_tables WHERE tablename = :collection_name')
    ...
    async def delete_collection(self, collection_name: str):
        async with self.db_client() as session:
            async with session.begin():
                ...
                drop_table = sql_text(f"DROP TABLE IF EXISTS {collection_name}")
    ...
    async def reset_vector_index(self, collection_name: str,
        ...
        return await self.create_vector_index(collection_name=collection_name, index_type=index_name)
    ...
    async def search_by_vector(self, collection_name: str, vector: list, limit: int) -> List[RetrievedDocument]:
        ...
        is_collection_existed = await self.is_collection_existed(collection_name=collection_name)
        if not is_collection_existed:
            self.logger.info(f"Can not search for a vector in a non-existed collection: {collection_name}")
            return False
