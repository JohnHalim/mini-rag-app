...
async def index_project(request: Request, project_id: int, push_request: PushRequest):
    ...
    porgress_bar = tqdm(total=total_chunks_count, desc="Vector Indexing", position=0)
    ...
    while has_records:
        ...
        porgress_bar.update(len(page_chunks))
...
async def search_index(request: Request, project_id: int, search_request: SearchRequest):
    ...
    search_result = await nlp_controller.search_vectordb_collection(
        project=project,
    ...
    return JSONResponse(
        content={
            ...
            "search_result" : [result.dict() for result in search_result]
        }
    )
